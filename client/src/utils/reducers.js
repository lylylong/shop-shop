...
export const reducer = (state, action) => {
  switch (action.type) {
    ...
    case UPDATE_PRODUCTS:
      return {
        ...state,
        // note: ...means everything remainds the same excerpt products
    ...
    case REMOVE_FROM_CART:
      let newState = state.cart.filter((product) => {
        // the use of the filter() method that only keeps the items that don't match the provided _id
        return product._id !== action._id;
      });
      return {
        ...state,
        // if newState.length > 0 cart is true，if newState.length = 0 so cart is false
        cartOpen: newState.length > 0,
        // original state should be treated as immutable
        cart: newState,
      };
